      ...
      for (const pos of slimmedPositions) {
        switch (pos.windowsKey) {
          case "Quests":
            ...
            break;
          case "Inventory":
            WindowController.getByName("Inventory").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "Logger":
            WindowController.getByName("Logger").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "Navigator":
            WindowController.getByName("Navigator").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "Vendor":
            WindowController.getByName("Vendor").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "Resources":
            WindowController.getByName("Resources").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "Events":
            WindowController.getByName("Events").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "Container":
            WindowController.getByName("Container").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
          case "UnitManagement":
            WindowController.getByName("UnitManagement").position = {
              x: pos.x,
              y: pos.y,
            };
            break;
